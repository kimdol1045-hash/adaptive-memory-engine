...
def test_benchmark_part4_multihop_reasoning_passes(tmp_path: Path, monkeypatch) -> None:
    engine = _part4_engine(tmp_path, monkeypatch)
...
def test_multihop_answer_exposes_reasoning_trace(tmp_path: Path, monkeypatch) -> None:
    engine = _part4_engine(tmp_path, monkeypatch)

    result = engine.query("왜 OpenClaw가 LightRAG를 사용하게 되었지?")

    assert result.raw["answer_builder"] == "multihop"
    assert result.raw["hops"] == ["GraphRAG 검토", "16GB 제약 확인", "LightRAG 채택", "OpenClaw Memory Layer 적용"]
    assert result.raw["reasoning_depth"] == 4
    assert len(result.sources) >= 3


def _part4_engine(tmp_path: Path, monkeypatch) -> QueryEngine:
    ...
    create_corpus("benchmark-part4")
    MemoryPipeline().ingest("benchmark-part4", source_dir, mode="deterministic")
    corpus_root = require_corpus("benchmark-part4")
...
def _benchmark_questions(path: Path) -> list[dict]:
    ...
    for index, match in enumerate(matches):
        ...
        questions.append(
            {
                "id": match.group(1),
                "question": _multiline_value(block, "Question"),
...
def _multiline_value(block: str, label: str) -> str:
    match = re.search(rf"^{re.escape(label)}:\s*\n(.*?)(?=^Expected Answer:|^## Q\d{{3}}\b|^# Category |\Z)", block, flags=re.M | re.S)
    if not match:
        return ""
