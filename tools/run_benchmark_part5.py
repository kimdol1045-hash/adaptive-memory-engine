...
DEFAULT_BENCHMARK = ROOT / "benchmark" / "adaptive_memory_engine_benchmark_part5_agent_memory_os.md"
DEFAULT_PRD = ROOT / "adaptive_memory_engine_prd_v2_2_integrated.md"
DEFAULT_MEMORY_MINI = ROOT / "tests" / "fixtures" / "memory-mini" / "docs"
DEFAULT_OUT = ROOT / "tests" / "out" / "sprint-5"


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Part 5 Agent Memory OS benchmark.")
    ...
    parser.add_argument("--corpus-id", default="benchmark-part5")
    ...
    for question in questions:
        ...
        missing = [expression for expression in question["must_contain"] if not present(expression, answer_text)]
        forbidden = [expression for expression in question["must_not_contain"] if present(expression, answer_text)]
