...
DEFAULT_BENCHMARK = ROOT / "benchmark" / "adaptive_memory_engine_benchmark_part2_timeline_supersedes.md"
DEFAULT_PRD = ROOT / "adaptive_memory_engine_prd_v2_2_integrated.md"
DEFAULT_MEMORY_MINI = ROOT / "tests" / "fixtures" / "memory-mini" / "docs"
DEFAULT_OUT = ROOT / "tests" / "out" / "sprint-2"


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Part 2 Timeline/SUPERSEDES benchmark.")
    ...
    parser.add_argument("--corpus-id", default="benchmark-part2")
    ...
    for question in questions:
        ...
        missing = missing_required(answer_text, question["required"])
        forbidden = present_forbidden(answer_text, question["forbidden"])
...
def benchmark_questions(path: Path) -> list[dict]:
    ...
    for index, match in enumerate(matches):
        ...
        required = bullet_section(block, "Must Contain")
        if not required:
            required = expected_lines(block)
        questions.append(
            {
                "id": match.group(1),
                "category": category,
                ...
                "required": required,
                "forbidden": bullet_section(block, "Must Not Contain"),
            }
        )
    return questions
...
def expected_lines(block: str) -> list[str]:
    lines = []
    for line in section(block, "Expected Answer").splitlines():
...
def missing_required(answer: str, required: list[str]) -> list[str]:
    return [expression for expression in required if not present(expression, answer)]


def present_forbidden(answer: str, forbidden: list[str]) -> list[str]:
    return [expression for expression in forbidden if present(expression, answer)]
